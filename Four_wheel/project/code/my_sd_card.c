...
//读写操作
uint8_t sd_spiReadWrite(uint8_t data)
{
    uint8_t res = 0;
    spi_transfer_8bit(SD_SPI_NUM, (uint8_t*)&data, &res, 1);
    return res;
}

//选择sd卡,并且等待卡准备OK   返回值:0,成功;1,失败       内部使用
uint8_t SdSelect(void)
{
...
//返回值:0,成功得到了该回应值
//其他,得到回应值失败    期待得到的回应值
SDHCRES SdGetResponse(uint8_t Response)
{
...
//  Sample usage:       内部调用
//-------------------------------------------------------------------------------------------------------------------
SDHCRES sd_sendcmd(uint8_t cmd, uint32_t arg, uint8_t crc)
{
    uint8_t r1;
    uint8_t Retry=0;
    SD_DisSelect();//取消上次片选
    ...
    sd_spiReadWrite((uint8_t)(arg >> 16) & 0xFF);
    sd_spiReadWrite((uint8_t)(arg >> 8) & 0xFF);
    sd_spiReadWrite((uint8_t)(arg) & 0xFF);
    sd_spiReadWrite(crc);
...
//  Sample usage:       内部调用
//-------------------------------------------------------------------------------------------------------------------
uint8_t sd_SendBlockData(uint8_t *buf, uint8_t cmd)
{
    uint16_t t;
...
//  Sample usage:       内部调用
//-------------------------------------------------------------------------------------------------------------------
SDHCRES sd_ReceiveData(uint8_t *buf,uint16_t len)
{
...
//cnt:扇区数
//返回值:0,ok;其他,失败.
SDHCRES sd_ReadDisk(uint8_t *buf,uint32_t sector,uint8_t cnt)
{
    uint8_t r1;
...
//cnt:扇区数
//返回值:0,ok;其他,失败.
SDHCRES sd_WriteDisk(uint8_t *buf,uint32_t sector,uint8_t cnt)
{
    uint8_t r1;
//    uint16_t retry;
...
//输入参数3   读取扇区的个数
//返回值是读取成功还是不成功
uint8_t  SD_read_sector_data(uint8_t data[], uint32_t sector, uint32_t count)
{
   uint16_t i;
   uint32_t j;
   uint32_t add=0;
...
//输入参数3  写入扇区的个数
//返回值是写入成功还是不成功
uint8_t  SD_write_sector_data(uint8_t data[], uint32_t sector, uint32_t count)
{
   uint16_t i;
   uint8_t response;
   uint32_t j;
